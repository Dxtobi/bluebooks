...
<section>
        ...
        <div class="w-full h-full absolute bg-[#2a2a2a51] backdrop-blur-[5px] z-30 ">
            <div class=" capitalize absolute left-0 right-0 top-0 bottom-0 m-auto z-40 min-w-[300px] min-h-[300px]  grid place-content-center">
               ...
               {#if book?.coverArtUrl === "null"}
                    <div class=" text-center object-contain shadow-md shadow-[#df87ff62] h-[200px] w-[130px] grid place-content-center bg-gray-300 rounded-md">
                        <h1> {book?.title}</h1>
                    </div>
                {:else}
    ...
    <section class="p-5 relativ overflow-scroll hide-scroll">
        ...
        <a href={`/read/${book?._id}`} class="w-[90%] p-3 rounded-lg bg-[#bbb] left-0 right-0 fixed bottom-2 m-auto md:w-1/3 text-center">Read for free</a>
    </section>
</section>
